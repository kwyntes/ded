...
// For making the titlebar dark
#ifdef _WIN32
#include <SDL2/SDL_syswm.h>
#include <dwmapi.h>

// For some reason this value is not in the header, eventhough Microsoft says it should be
// https://learn.microsoft.com/en-us/windows/win32/api/dwmapi/ne-dwmapi-dwmwindowattribute
// It's probably in Windows 11 so I added two underscores to not conflict in that case.
const int DWMWA_USE_IMMERSIVE_DARK_MODE__ = 20;
#endif
...
int main(int argc, char **argv)
{
    ...
    SDL_Window *window =
        SDL_CreateWindow("ded",
                         ...
                         SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN // Making the titlebar dark on Windows requires rerendering the window if it's shown already before DwmSetWindowAttribute is called
        );
    if (window == NULL)
    {
...
#ifdef _WIN32
    {
        ...
        SDL_GetWindowWMInfo(window, &wminf);
    ...
    // Show the window after DwmSetWindowAttribute
